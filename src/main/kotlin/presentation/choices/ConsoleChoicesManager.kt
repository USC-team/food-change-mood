...
class ConsoleChoicesManager(private val getGuessGameUseCase: GetGuessGameUseCase,
                            ...
                            private val iMessagesPrinter: MessagePrinter,
    ...
    private fun isCorrectGuess(meal: Meal, tries: Int=3): Boolean {
       if (tries > 0) {
            ...
            val guessResult = getGuessGameUseCase.isGuessCorrectHighOrLow(meal, readManager.readInt())
            when (guessResult) {
                GuessResult.Correct -> {
    ...
    private fun noEggsSweet(){
        ...
        if(didYouLikeIt(readManager.readLine())) iMessagesPrinter.showMealsDetails(meal)
        else noEggsSweet()
    }
    ...
    private fun getKetoMeal(){
        ...
        iMessagesPrinter.printMealName(meal)
        iMessagesPrinter.askUserIfLikedMeal()
        followKeto(readManager.readLine(), meal)
    }
